...
Multiple example configurations are provided in the [tsconfig directory].
These tsconfig expect an [auth key] to be set in the `TS_AUTHKEY` environment variable.
All nodes registered while running these tsconfig will be ephemeral and removed after disconnect.
See the comments in the individual files for details.

Run them with:

```sh
TS_AUTHKEY=<tskey-auth-XXXXX> ./caddy run -c tsconfig/<file>
```
...
```sh
docker run -it -rm ghcr.io/sudosu404/tailnet-proxy
```
...
docker run -it -rm \
  ...
  ghcr.io/sudosu404/tailnet-proxy
```
...
[tscaddy.App]: https://pkg.go.dev/github.com/sudosu404/tailnet-proxy#App
